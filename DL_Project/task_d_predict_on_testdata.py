...
def save_ensemble_predictions(predictions, test_loader, prediction_path='./ensemble_predictions.csv'):
    ...
    for i in range(len(test_loader.dataset)):
        img_path = test_loader.dataset.data[i]['img_path']  # Adjust key as per dataset
...
def get_test_predictions(models_dict, test_loader, device, ensemble_method='weighted_average', output_path='ensemble_predictions.csv'):
    """
    Get predictions on test dataset using specified ensemble method
    """
    ensemble = EnsembleMethods(models_dict, device)
    
    # Get predictions from all models on test data
    print(f"\nGetting test predictions using {ensemble_method}...")
    test_predictions = {}
    all_image_ids = []
    
    # First collect all image IDs
    for idx in range(len(test_loader.dataset.data)):
        if isinstance(test_loader.dataset.data[idx], dict):
            # Handle both single and dual image cases
            if 'img_path' in test_loader.dataset.data[idx]:
                image_id = os.path.basename(test_loader.dataset.data[idx]['img_path'])
            else:
                # For dual image case, use the first image path
                image_id = os.path.basename(test_loader.dataset.data[idx]['img_path1'])
            all_image_ids.append(image_id)
    ...
    for model_name, model in ensemble.models.items():
        ...
        with torch.no_grad():
            for batch in tqdm(test_loader, desc=f'Getting predictions for {model_name}'):
                # For test dataset, we only have images (no labels)
    ...
    if ensemble_method == 'weighted_average':
        weights = {model_name: 1/len(ensemble.models) for model_name in ensemble.models.keys()}
        ensemble_preds = np.zeros_like(list(test_predictions.values())[0])
        ...
        for _ in range(n_bags):
            sampled_models = np.random.choice(
                list(test_predictions.keys()),
                size=len(test_predictions),
                replace=True
            )
            bag_preds = np.zeros_like(list(test_predictions.values())[0])
            for model_name in sampled_models:
                bag_preds += test_predictions[model_name]
            bag_preds /= len(sampled_models)
            all_preds.append(np.argmax(bag_preds, axis=1))
    ...
    elif ensemble_method in ['stacking', 'boosting']:
        print(f"Warning: {ensemble_method} requires training data. Using weighted average instead.")
        weights = {model_name: 1/len(ensemble.models) for model_name in ensemble.models.keys()}
        ensemble_preds = np.zeros_like(list(test_predictions.values())[0])
        for model_name, preds in test_predictions.items():
            ensemble_preds += weights[model_name] * preds
    ...
    assert len(all_image_ids) == len(final_preds), f"Length mismatch: {len(all_image_ids)} IDs vs {len(final_preds)} predictions"
    ...
    print(f"\nSaved test predictions to {os.path.abspath(output_path)}")
...
if __name__ == '__main__':
    ...
    test_dataset = RetinopathyDataset('./DeepDRiD/test.csv', './DeepDRiD/test/', transform_test, 'single', test=True)
    test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False)
    
    # Get predictions using the best performing ensemble method
    get_test_predictions(
        models_dict,
        test_loader,
        device,
        ensemble_method='bagging',
        output_path='ensemble_predictions_bagging.csv'
    )